...
void myexpm::lsolve(bmatrix &A, bmatrix &BX){
    int n = A.size1();
    int nrhs = n;
    vector<int> ipiv(n);
    int info;
...
void myexpm::balanceMatrix(bmatrix &A, vector<double> &scale, int &ilo, int &ihi, bool Lbalance) {
...
void myexpm::expm_higham05(bmatrix &AeA) {
...
void myexpm::expm_najfeld_havel(bmatrix &AeA, bool doDP=true) {
